...
Ticket 05 records narrowly scoped decisions discovered after the initial delivery:

- A tag mutation needs explicit, case-insensitive per-tag add/remove intents so a later intent can supersede only the failed intent it covers.
- A failed intent persists for the application session even while its photo is absent from the Photo Workspace. It reappears as a non-alarming attention marker if the photo returns.
- Failed intents remain manual retries; TAGGER neither automatically retries them nor persists them across restart.
- Retry replays TAGGER's intent over freshly read metadata. TAGGER's intent wins for its addressed tags, while unrelated external metadata changes survive.
- This follow-up must not become a durable mutation queue, conflict-resolution system, or general MainWindow refactor.

## Comments

The original AI-generated implementation brief duplicated this completed specification and retained unchecked acceptance boxes after all five implementation tickets were completed. It was removed during tracker cleanup; completion evidence remains in `.scratch/reliable-tag-mutations/issues/`.
